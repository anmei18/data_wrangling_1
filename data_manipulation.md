...
Instead of saying what we want to keep, we can specify what we dont want
using `-`
...
\#Select helpers : ?select\_helpers
...
select litter\_number as the first column and keep everything else
...
same as the above func, it relocates litter\_number as the first column
and keep the rest
...
`Select` is the key function, but there are funcs that can duplicate to
some of the things that you can do with select

## `filter`

`filter` is intended to remove rows or to keep rows that you are
interested in retaining for some subsequent analysis

`select` operates on columns while `filter` operates on rows

Filter the data with the weight of mother on the first day that she’s
pregnant that is less than 22
...
Get all of the data where gd\_of\_birth is not 20. This works on more
complicated things
...
options(tibble.print\_max = Inf) \# to show all the rows.
...
first arrange in order by pups\_born\_alive and then within each of
those levels its in order for gd0\_weight
